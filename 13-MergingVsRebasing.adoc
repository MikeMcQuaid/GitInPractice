...
* `master`: Seen in figure indirectly; merged in `Merge 'branch' master into next` commit. An _integration branch_ used for preparing a new release; development is merged into here. New feature branches are branched off of `master`.
* feature branches: Seen in figure merged `Merged topic '...' into next` commits. Used for development of all bug fixes and new features. All new commits (except merge commits) are made on feature branches. Merged into `next` and `master` for integration testing.
