...
//(TP)-and rereading and rereading and ... :) Excellent!
//(TP)-Is the line below not a repeat of the lines above?
...
//(TP)-Can you write 'git clone --depth 0'? And what happens if the integer is negative?
...
//(TP)-Can you have spaces in your branch names?
...
#### Solution
//(TP)-Will anything happen if you run 'git checkout <name>' if you are already on the <name> branch?
1.  Change directory to the Git repository e.g. `cd /Users/mike/GitInPracticeRedux/`.
...
//(TP)-I think step 5 should be stressed more, that is you "stand" on the branch you wish to merge into (you kind of pull the changes instead of the other way around, where you stand on the branch you wish to merge from and push the changes)
#### Solution
1.  Change directory to the Git repository e.g. `cd /Users/mike/GitInPracticeRedux/`.
2.  Run `git checkout chapter-two` to ensure you are on the `chapter-two` branch.
