...
// Don't start with a note, always start with text.
...
on a server), sample _hooks_ (scripts that run on defined events e.g.
...
// turn into bulleted list instead and use (1) to refer to listing
...
// git checkout wants an action verb like the others
...
// add commands before all listings
----
# On branch master <1>
#
# Initial commit <2>
...
// 2.12 doesn't have the trailing 7 in it?
