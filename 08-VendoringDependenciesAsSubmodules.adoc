...
//(TP)-Submodules seems very similar in functionality to nuget-packages for Visual Studio. Would it make sense to compare the two or is that to specific to Windows/VS?
...
//(TP)-Is the submodul added to all branches or only the current one?
#### Solution
...
//(TP)-Thinking of the previous chapter on Personalizing Git, especially the aliasing, it would be great with some examples here. If there are sequences relating to submodules you always do, consider showing the alias or linking of command.