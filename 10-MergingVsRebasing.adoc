...
// Have examples initially
// explain both strategies first before pros and cons of either
// 10.1 Why I should use merging
// How branching/merging in CMake fails (rather than just succeeds)
// Need an example of bad things happening
// Homebrew and CMake both need separate workflow diagrams for how branches are handled
// Discuss Homebrew having very few people needed with commit access
// Dates aren't contiguous in either
// Newsnight debat vs Newsround article; more Paxman needed!

// I don’t know if the dichotomy is really “should you rebase/squash or should you branch” so much as it is “what should your branching strategy be?”

// The way it was written, I got the impression that rebasing and squashing hadn’t really been covered yet - but if not, they probably should be.
...
// What do you do based on these answers?
