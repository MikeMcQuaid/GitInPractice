...
To push all the unpushed commits on the current branch to a Subversion repository you use the `git svn dcommit` command. If you'd committed to the current branch of a `git svn` repository and ran `git svn dcommit` the output should resemble:
