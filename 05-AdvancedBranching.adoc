...
system.
```
Now the merge conflict has been resolved the merge conflict can be marked as resolved with `git add` and then the merge commit committed. You do not need to run `git merge` again; you're still in the middle of a merge operation which concludes when you `git commit`.
...
If you had run `git describe --tags` when on the previous commit (which was the `v0.1` tag) it would have outputted just `v0.1`.

You have generated a version number based on the existing tags in the repository.

#### Discussion
If `git describe` is passed a ref then it will generate the version number for that particular commit. For example, `git describe --tags v0.1` will output `v0.1` and `git describe --tags 0a5e328` will output `v0.1-1-g0a5e328.`
