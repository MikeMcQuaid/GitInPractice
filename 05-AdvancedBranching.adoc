...
//(TP)-Maybe a few word on the rational behind tags not being pushed 'automatically' by 'git push'. 
...
//(TP)-Might be slightly off topic, but it would be very interesting to learn how Git could help you with the semantic versioning. If there is time and room it would add great value to know how to 'tag' minor vs. major releases.
...
//(TP)-If people have begun to use the short '-m' for message, the version number will not contain the hash. Maybe a comment about this?
...
//(TP)-Don't you have to do a 'git add 02-AdvancedGitInPractice.asciidoc' before the commit?
1.  Change to the directory containing your repository e.g. `cd /Users/mike/GitInPracticeRedux/`.
