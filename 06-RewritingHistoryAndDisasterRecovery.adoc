...
You can from Figure 6.1 that the SHA-1s match those in the `git reflog` output:
...
Figure 6.9 shows the partial output from GitX after the `git clone` command focusing on the `master` branch. Now let's remove references to the preface file from the `master` branch.
