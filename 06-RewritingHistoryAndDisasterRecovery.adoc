...
You can also perform a combined reset-and-commit operation to modify the previous commit using `git commit --amend`. `git commit --amend` resets to the previous commit and then creates a new commit with the same commit message as the previous. It uses `git reset --soft HEAD^` and then runs `git commit --reedit-message` with the now-reset commit as an argument. This in effect means that it'll add anything you have currently added to the index staging area to the changes of the previous commit and prompt for a new commit message. I most commonly use this to adjust the previous commit message if I realize I've made a typo or omitted useful information.
