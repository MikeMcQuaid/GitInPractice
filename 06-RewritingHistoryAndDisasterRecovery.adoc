...
//(TP)-Would you ever create an empty commit manutally? (except to explain something, of course). Maybe an example or explanation?
1.  Change to the directory containing your repository e.g. `cd /Users/mike/GitInPracticeRedux/`.
2.  Run `git checkout inspiration`.
...
//(TP)-Does it make any difference if you have multiple 'local' commits before running 'git pull --rebase'? If so an explanation might be in order.
#### Discussion
