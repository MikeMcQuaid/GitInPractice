...
//(TP)-It possible, please consider to add something about how to find the "active" configuration value. Let's say you try to do something and get an unexpected result because the configuration value had been "overwritten". How do you determine where to look for the "incorrect" value? Sure, you could always set is locally, but maybe you don't want to do this.
...
```
//(TP)-May have been explained in a previous chapter, but worth repeating: 'git version' to determine the version of your Git installation.
...
# grep --before=1 ultimate ~/.gitconfig
//(TP)-I must be doing something wrong, because I get 'Expansion of alias 'l' failed; 'ultimate-log' is not a git command' when I try to run 'git l' after git config --global alias.l ultimate-log'. 'git ultimate-log' works fine.
[alias]
...
//(TP)-The link to the dotfiles does not work. 404 error.
...
//(TP)-If you don't know what terminal you are using, maybe a few words on how to find out.
#### Solution
