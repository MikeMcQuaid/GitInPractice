...
// Needs more annotations. Always refer to figures in text at least once.
// Figures and listings should be nearer their text. Look at PDF output.
...
// Way more abstract than 1.1 or 1.2. Needs to be a lot more like Figure 1.2.
// Should reference more stuff, have a running commentary round the outside,
// better caption.
...
// Not clear why arrows are different ways round. Timeline from left to right?
// See 1.5 for why arrows are pointing this way? Also, should be down next to
// the reference. You can have bugfixes for books. Also explain why you might
...
// Update the text here to reflect that it's a book not code.
...
// "A typical commit broken down into its component parts is shown in Figure
// 1.5" and then figure here.
...
we will do in Chapter 2) you would find objects, various pointers/references to
objects and configuration files.
// Reference Figure 1.6 in here.
...
// Signify movement on first three commits. Perhaps join them with a dotted line
// or arrows? Line up "add new button" commits.
...
unnecessarily? In Figure 1.8 the commits are being _squashed_ together so
...
The "(1) diff command" shows an example `diff` command that may have been used
to output these changes (although in reality this is done by Git internally).
The "(2) staging area SHA-1" shows the changes that were made to the `index
...
"(3) old file version" and "(4) new file version" show virtual file names
...
folders (`a` and `b` were compared). The "(5) file changes range" is used by
...
should be changed. Lines that are "(6) unchanged" are displayed as-is, "(7)
deleted" lines are prefixed with a `-` and "(8) inserted" lines are prefixed
with a `+`.
